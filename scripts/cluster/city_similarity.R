...
input_data= args[[1]]   # first arg - name of dataset

#set to TRUE if you want to also calculate sample similarity only using relevant markers for the 'city' variable...
#... found by feature selection
#NOTE this only makes sense if the marker data is discretized
...
#functions to compute similarities

#plain cosine in terms of original numbers in "mtrx"
...
# setting up different variants of similarity calc
...
source('helpers.R')
source('click-que.R')	
## function 2 find threshold of simil. at which # of cliques of given min. size is maximal
...
### table w/ columns:
#### variant name, statistic value, number of samples*
#### *since sometimes we have to remove samples with 0 cosine norm
####     keep track of this number to see if this particular dataset
...
##### change theese lines if you wish to have different filenames of results

write.csv( S_io_df, paste0(input_data,'similarity_stats_results.csv'))

##### do not comment theese lines and do not remove theese files
##### if you wish to create heatmap out of theese
##### results later

write.csv(classCityLookup, paste0(input_data,"CityCodes.csv"))
saveRDS(variousSimilarities, 'vS.rds')
saveRDS(classVector, 'cV.rds')
