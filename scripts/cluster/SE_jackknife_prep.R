### preparation of  leave 0.25 out from each city jackknife resampling
### beginning is just like in city_similarity.R
### then, based on cleaned dataset, replicates are drawn
### keeping the city proportions

source('leave_percent_out_multiclass.R')
...
set.seed(123)
leaveFracOutMulticlassSampleEachSubset( classVector=classVector,
			 frac=0.25, n_replicates=500) -> reps

saveRDS(reps, "LeaveQrtReps.rds")
